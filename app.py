...
Original file is located at
    https://colab.research.google.com/drive/1Hs2WvWMOXVbAQfEqKgtIbDDQYaX0wvjV
"""
...
# STREAMLIT UI
# -------------------
st.set_page_config(page_title="My Independence Life Timer", page_icon="🇮🇳", layout="centered")

# Flag banner
st.image("https://upload.wikimedia.org/wikipedia/en/4/41/Flag_of_India.svg", width=200)
st.markdown("<h1 style='text-align:center; color:orange;'>🇮🇳 Independence Day Life Timer 🇮🇳</h1>", unsafe_allow_html=True)
...
if birth_date:
    ...
    # -------------------
    # HEADER
    st.markdown(f"<h2 style='color:orange;'>🎉 Celebrating {independence_years}th Independence of Motherland INDIA 🇮🇳</h2>", unsafe_allow_html=True)

    # SECTION 4
    st.markdown(f"<h3 style='color:orange;'>MESSAGE FROM THE MOTHERLAND</h3>", unsafe_allow_html=True)
    st.write(random.choice(messages))

    # SECTION 2
    st.markdown(f"<h3 style='color:orange;'>DAYS PASSED BEFORE YOUR BIRTH</h3>", unsafe_allow_html=True)
    if days_after_independence >= 0:
        st.write(f"India was independent for **{before_birth_diff.years} years, {before_birth_diff.months} months, and {before_birth_diff.days} days** before you were born.")
    else:
        st.write(f"You were born **{-days_after_independence} days** before India’s Independence "
                 f"({before_birth_diff.years} years, {before_birth_diff.months} months, {before_birth_diff.days} days).")

    # SECTION 3
    st.markdown(f"<h3 style='color:white; background-color:green; padding:5px;'>DAYS YOU HAVE LIVED IN INDEPENDENT BHARAT</h3>", unsafe_allow_html=True)
    st.write(f"You have spent **{independent_days_lived} days** "
             f"({independent_life_diff.years} years, {independent_life_diff.months} months, {independent_life_diff.days} days) in Independent Bharat 🇮🇳")

    # SECTION 1
    st.markdown(f"<h3 style='color:green;'>YOUR BIRTH INFORMATION</h3>", unsafe_allow_html=True)
    st.write(f"Years Completed: **{years_completed} years**")
    st.write(f"Decades: **{decades} decades and {extra_years_in_decade} years**")
    st.write(f"Months Completed: **{months_completed}**")
    st.write(f"Days Since Birth: **{days_since_birth}**")
    st.write(f"Leap Years Passed: **{leap_years}**")
    st.write(f"Hours Since Birth: **{hours_since:,}**")
    st.write(f"Minutes Since Birth: **{minutes_since:,}**")
    st.write(f"Seconds Since Birth: **{seconds_since:,}**")
